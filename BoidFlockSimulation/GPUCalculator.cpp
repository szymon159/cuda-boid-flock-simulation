...
void GPUCalculator::updateSeparationFactor(float2 &separationFactor, const float2 &startBoidPosition, const float2 &targetBoidPosition, const float &distance)
{
...
void GPUCalculator::updateAlignmentFactor(float2 &alignmentFactor, const float2 &targetBoidVelocity)
{
	alignmentFactor.x += targetBoidVelocity.x;
	alignmentFactor.y += targetBoidVelocity.y;
}

void GPUCalculator::updateCohesionFactor(float2 &cohesionFactor, const float2 &targetBoidPosition)
{
	cohesionFactor.x += targetBoidPosition.x;
	cohesionFactor.y += targetBoidPosition.y;
}

float2 GPUCalculator::normalizeVector(float2 &vector)
{
...
float GPUCalculator::normalizeAngle(float &angle)
{
	return (float)fmod(angle, 360);
}

float2 GPUCalculator::getVectorFromAngle(float angle)
{
	float2 result;
...
float GPUCalculator::getAngleFromVector(float2 vector)
{
...
float GPUCalculator::calculateDistance(float2 startPoint, float2 targetPoint)
{
...
float2 GPUCalculator::getMovementFromFactors(float2 separationVector, float2 alignmentVector, float2 cohesionVector, float refreshRateCoefficient)
{
	float2 movement;
	//float angle;
